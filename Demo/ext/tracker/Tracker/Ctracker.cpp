...
// ---------------------------------------------------------------------------
CTrack::CTrack(Rect rect, float dt, float Accel_noise_mag)
{
	bounding_box = rect;
	detect = false;
	Point2d pt = CTracker::rect2point(rect);
...
//
// ---------------------------------------------------------------------------
Point2d CTracker::rect2point(Rect &rect)
{
	return Point2d(rect.x + (rect.width >> 1), rect.y + (rect.height >> 1));
}
void CTracker::Update(vector<Rect>& detections)
{
	// -----------------------------------
	...
	// -----------------------------------
	// Here the tracks already there anyway
	// -----------------------------------
	int N=tracks.size();		// треки
	int M=detections.size();	// детекты

	// Array of distances from the N-tion track before M-tion detective.
	vector< vector<double> > Cost(N,vector<double>(M));
	vector<int> assignment; // назначения

	// -----------------------------------
	// Tracks already have, form the matrix of distances
	// -----------------------------------
	double dist;
	...
	{	
		...
		{
			Point2d diff=(tracks[i]->prediction-rect2point(detections[j]));
	...
	for(int i=0;i<assignment.size();i++)
	{
		if(assignment[i]!=-1)
		{
			...
			{
				tracks[i]->bounding_box = detections[assignment[i]];
				tracks[i]->detected_frames++;
				tracks[i]->detect = true;
			}
		}
		else
		{			
	...
	for(int i=0;i<assignment.size();i++)
	{
		...
		if(assignment[i]!=-1) // If we have assigned detect, then update using its coordinates,
		{
			...
			tracks[i]->detected_frames++;
			tracks[i]->prediction=tracks[i]->KF->Update(rect2point(detections[assignment[i]]),1);
		}else				  // if not continue using predictions
		{
			tracks[i]->prediction=tracks[i]->KF->Update(Point2f(0,0),0);	
			tracks[i]->detect = false;
		}
		...
		tracks[i]->bounding_box.x = int(tracks[i]->prediction.x) - (tracks[i]->bounding_box.width >> 1);
		tracks[i]->bounding_box.y = int(tracks[i]->prediction.y) - (tracks[i]->bounding_box.height >> 1);
	}
