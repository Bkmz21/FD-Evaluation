...
for i = 1:size(min_size, 2);
    ...
    if use_FDDB_eval
        [data, label, color, marker, data_label] = evalFDDB(alg, msz, sf, knn);
    else
    ...
    %% Calculation metrics
    if ~use_FDDB_eval    
        [true, false, recall, precision, F_score] = calcMetrics(data, F_alpha, @mean);
    else
        [true, false, recall, precision, F_score] = calcMetrics(data, F_alpha, @mean, -1, 1, 2);        
    end
    ...
    if use_F_score
        name = ['F-score for FDDB-' int2str(msz)];        
        graphData(F_score, label, color, marker, [], name);
    else
        if use_ROC || use_FDDB_eval
            name = ['ROC-curve for FDDB-' int2str(msz)];
            graphData({recall, false}, label, color, marker, {'Recall', 'False positive'}, name);   
        else
            name = ['PR-curve for FDDB-' int2str(msz)];
